 ...
 */
(function () {
    // Optional: dynamically load a task-specific JS based on taskId in URL
    try {
        var __taskId = (Alfresco.util.getQueryStringParameter("taskId") || "").trim();
        if (__taskId) {
            // Build a safe suffix, e.g. "activiti$3349" -> "activiti_3349"
            var __suffix = __taskId.replace(/\$/g, "_");
            // Optional sanity check to avoid arbitrary paths
            if (/^activiti_\d+$/.test(__suffix)) {
                var __extraSrc = "/com/acibfunin/components/workflow/workflow-form-" + __suffix + ".js";
                if (window.YAHOO && YAHOO.util && YAHOO.util.Get && YAHOO.util.Get.script) {
                    YAHOO.util.Get.script(__extraSrc, {
                        onSuccess: function () {
                            if (window.console && console.debug) {
                                console.debug("Loaded task-specific script:", __extraSrc);
                            }
                        },
                        onFailure: function () {
                            if (window.console && console.warn) {
                                console.warn("Optional task-specific script not found:", __extraSrc);
                            }
                        }
                    });
                }
        ...
        if (window && window.console && console.warn) {
            console.warn("Error while attempting to load task-specific script:", e);
        }
    }
    /**
     * YUI Library aliases
    ...
    Alfresco.component.WorkflowForm = function WorkflowForm_constructor(htmlId) {
        ...
        // custom code to replace the ObjectFinder table in the "Items" set with a custom table
        YAHOO.Bubbling.on("workflowFormReady", function () {
            // --- 1) Hide original "Items" set ---
            var itemsSet = null;
            document.querySelectorAll(".set").forEach(function (set) {
                var title = set.querySelector(".set-title");
                if (title && (title.textContent.trim().toLowerCase() === "items" || title.textContent.trim().toLowerCase() === "elementos")) {
                    set.style.display = "none";
                    itemsSet = set;
                }
            });

            // Hide packageItems control explicitly
            document.querySelectorAll(
                "[id*='_assoc_packageItems'], [name*='_assoc_packageItems'], .object-finder-list"
            ).forEach(function (el) { el.style.display = "none"; });

            // --- 2) Create our replacement section ---
            var wrapper = document.createElement("div");
            wrapper.className = "set";

            var titleDiv = document.createElement("div");
            titleDiv.className = "set-title";
            titleDiv.textContent = "Docs";

            var formField = document.createElement("div");
            formField.className = "form-field";

            var container = document.createElement("div");
            container.id = "ks-task-entries";
            container.className = "viewmode-value current-values";
            container.innerHTML = '<div style="opacity:.7">Cargando…</div>';

            formField.appendChild(container);
            wrapper.appendChild(titleDiv);
            wrapper.appendChild(formField);

            var formFields = document.querySelector(".form-fields") || document.body;
            if (itemsSet && itemsSet.parentNode) {
                itemsSet.parentNode.insertBefore(wrapper, itemsSet.nextSibling);
            } else {
            ...
            if (/^\d+$/.test(taskId)) taskId = "activiti$" + taskId;
            var url = Alfresco.constants.PROXY_URI + "task/info?taskId=" + encodeURIComponent(taskId);

            // --- 4) Ajax to paint the table (targets #ks-task-entries) ---
            Alfresco.util.Ajax.request({
                ...
                successCallback: {
                    fn: function (resp) {
                        try {
                            ...
                            var html = '<div class="viewmode-field">';

                            if (!entries.length) {
                                html += '<div style="opacity:.8">(sin entradas)</div>';
                            } else {
                                var table =
                                ...
                                entries.forEach(function (e) {
                                    ...
                                    table +=
                                        '<tr class="yui-dt-rec">' +
                                        '  <td class="yui-dt-col-nodeRef yui-dt-first">' +
                            ...
                            html += '</div>';
                            document.getElementById("ks-task-entries").innerHTML = html;
                        } catch (e) {
                            document.getElementById("ks-task-entries").innerHTML =
                                '<div style="color:#b00">Error parseando JSON</div>';
                ...
                failureCallback: {
                    fn: function (resp) {
                        ...
                        document.getElementById("ks-task-entries").innerHTML =
                            '<div style="color:#b00">Error ' + status + ' llamando al webscript</div>';
                    }
                }
            });
        });
            ...
            onWorkflowFormLoaded: function WorkflowForm_onWorkflowFormLoaded(response) {
                ...
                var currentTasksColumnDefinitions =
                    [
                        ...
                        {
                            key: "owner",
                            label: this.msg("column.assignedTo"),
                ...
                var historyColumnDefinitions =
                    [
                        ...
                        {
                            key: "owner",
                            label: this.msg("column.completedBy"),
                            formatter: this.bind(this.renderCellCompletedBy)
                        },
                        {
                            key: "id",
                            label: this.msg("column.dateCompleted"),
